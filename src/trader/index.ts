...
export class TradingAgent {
  ...
  constructor(storage: Storage, timeframe: Timeframe) {
    this.watchlist = new WatchlistManager();
    this.storage = storage;
    this.timeframe = timeframe;
    this.paperTrader = new PaperTrader(timeframe, storage);

    logger.info('TRADER', `🤖 Paper Trading Agent [${timeframe}m] initialized`);
    logger.info('TRADER', `   Strategy: EMA Pullback (9/15 EMA on ${timeframe}m chart)`);
    logger.info('TRADER', `   Initial Balance: $10,000 | Risk: 2% per trade | R:R 1:2`);
   ...
   */
  start(): void {
    logger.info('TRADER', `▶️ [${this.timeframe}m] Trading agent started — polling every 60s for pullback setups`);

    // Main polling loop
    this.pollTimer = setInterval(() => {
      this.tick().catch(err => {
        logger.error('TRADER', `[${this.timeframe}m] Tick error`, { error: String(err) });
      });
    }, POLL_INTERVAL_MS);
  ...
  stop(): void {
    ...
    logger.info('TRADER', `⏹️ [${this.timeframe}m] Trading agent stopped`);
  }
  ...
  private async tick(): Promise<void> {
    ...
    for (const entry of entries) {
      if (!this.paperTrader.hasPosition(entry.symbol)) {
  ...
  private printStatus(): void {
    ...
    logger.info('TRADER', `🤖 PAPER TRADING STATUS [${this.timeframe}m Chart]`);
    logger.info('TRADER', `   Simulated Capital: $${stats.balance.toFixed(2)}`);
